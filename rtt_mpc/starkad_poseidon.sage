...
class HadesParams(object):
    def __init__(self, field, r, c, Rf, Rp, alpha=3, rcs=None, mds=None, lsbox=True, reduced=False):
        ...
        if rcs is not None:
            self.ark = [vector(field, rcs[i*m:(i+1)*m]) for i in range(n_rounds)]
        else:
        ...
        if mds is not None:
            self.mds = matrix(field, m, m)
            for i in range(0, m):
                for j in range(0, m):
                    self.mds[i, j] = field(mds[i][j])
...
def hades_round(values, params, is_full_round, round_idx):
    ...
    if is_full_round:
        for i in range(len(values)):
            values[i] = values[i] ** params.alpha
    elif params.lsbox:
        values[-1] = values[-1] ** params.alpha
    else:
        values[0] = values[0] ** params.alpha
    # MixLayer
...
def test():
    ...
    # Poseidon S128f test vector, simple
    test_inp = vector(Poseidon_S128_BLS12_138.field, [0, 1, 2, 3, 4])
    test_res = hades_permutation(test_inp, Poseidon_S128_BLS12_138)
    exp_res = [0x2a918b9c9f9bd7bb509331c81e297b5707f6fc7393dcee1b13901a0b22202e18,
